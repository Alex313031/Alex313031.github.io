...
var rolloverMesh;
var sampleMat;
var rollOverObject, rollOvsplierObject2;
var timer = 0;
...
function getGeometry() {
    number = {random: new Math.seedrandom(Math.random())};

    var geometry;

    var type = Math.floor(number.random() * 9);
    var size = 10 + number.random() * 15;
    var quality = number.random()
    var mod = number.random()

    var bugs = number.random() < .5;

    if (bugs)
        geometry = geoms[Math.floor(number.random() * geoms.length)]
    ...
    if (bugs) {
        ...
        geometry.applyMatrix(new THREE.Matrix4().makeRotationX(number.random() * Math.PI));
        geometry.applyMatrix(new THREE.Matrix4().makeRotationY(number.random() * Math.PI));
        geometry.applyMatrix(new THREE.Matrix4().makeRotationZ(number.random() * Math.PI));
    } else {
        if (type == 0) {
            geometry = new THREE.BoxGeometry((.25 + 1.5 * quality) * size, (.25 + 1.5 * (1 - quality)) * size, (.25 + 1.5 * mod) * size);
        }

        if (type == 1) {
            var matrix = new THREE.Matrix4()
            matrix.makeRotationFromEuler(new THREE.Euler(number.random() * Math.PI, number.random() * Math.PI, number.random() * Math.PI))
            geometry = new THREE.OctahedronGeometry(size, Math.floor(quality * 2))
        ...
        if (type == 2) {
            var matrix = new THREE.Matrix4()
            matrix.makeRotationFromEuler(new THREE.Euler(number.random() * Math.PI, number.random() * Math.PI, number.random() * Math.PI))
            geometry = new THREE.TetrahedronGeometry(size, Math.floor(quality * 3))
            geometry.applyMatrix(matrix);
        }
        ...
        if (type == 3) {
            var matrix = new THREE.Matrix4()
            matrix.makeRotationFromEuler(new THREE.Euler(number.random() * Math.PI, number.random() * Math.PI, number.random() * Math.PI))
            geometry = new THREE.IcosahedronGeometry(size, Math.floor(quality * 2))
            geometry.applyMatrix(matrix);
        }
        ...
        if (type == 5) {
            ...
            geometry = new THREE.CylinderGeometry(size * .7, size * .7, size * 1.5, 3 + Math.floor(mod * 6));
            geometry.applyMatrix(matrix);
        }
        ...
        if (type == 6) {
            ...
            if (quality * 30 < 5)
                tsize *= 1.5
            geometry = new THREE.CircleGeometry(tsize, Math.floor(quality * 30))
            geometry.applyMatrix(matrix);
        }
        ...
        if (type == 7) {
            ...
            geometry = new THREE.TorusGeometry(size * .8, size * (mod * .7 + .1), 2 + Math.floor(quality * 15), 3 + Math.floor(quality * 20));
        ...
        if (type == 8) {
            var matrix = new THREE.Matrix4()
            if (mod < .33) {
                matrix.makeRotationFromEuler(new THREE.Euler(0, 0, Math.PI))
            } else {
                if (mod < .5) {
                    matrix.makeRotationFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0))
                }
            ...
            geometry = new THREE.ConeGeometry(1.4 * size, size * (.7 + 1 * mod), 3 + Math.floor(quality * 20));
        ...
        if (type == 9) {
            var matrix = new THREE.Matrix4()
            matrix.makeRotationFromEuler(new THREE.Euler(number.random() * Math.PI, number.random() * Math.PI, number.random() * Math.PI))
            geometry = new THREE.DodecahedronGeometry(size, Math.floor(mod * 2))
            geometry.applyMatrix(matrix);
        }


        var color = new THREE.Color()
        color.setHSL(mod, .5, .5)

        var color2 = new THREE.Color()
        color2.setHSL(quality, .5, .5)
        colorGeometry(geometry, color, color2)
    }

    return geometry;
}

function colorGeometry(geometry, color, color2) {
    var vertexIndex;
...
function addGeom(geom, rotationArray, translationArray, scaleArray) {
    ...
    geom.applyMatrix(new THREE.Matrix4().makeRotationX(Math.random() * Math.PI));
    geom.applyMatrix(new THREE.Matrix4().makeRotationY(Math.random() * Math.PI));
    geom.applyMatrix(new THREE.Matrix4().makeRotationZ(Math.random() * Math.PI));
