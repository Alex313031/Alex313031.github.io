...
WAGNER.Composer = function( renderer, settings ) {
	...
	this.quad = new THREE.Mesh(
		THREE.PlaneBufferGeometry ? new THREE.PlaneBufferGeometry( 1, 1 ) : new THREE.PlaneGeometry( 1, 1 ),
...
WAGNER.Composer.prototype.render = function( scene, camera, keep, output ) {
	...
	if( this.copyPass.isLoaded() ) {
		...
		this.renderer.render( scene, camera, output?output:this.write );
...
WAGNER.Composer.prototype.pass = function( pass ) {
	...
	if( pass instanceof WAGNER.Stack ) {
		...
		if( pass instanceof THREE.RawShaderMaterial || pass instanceof THREE.ShaderMaterial ) {
...
WAGNER.Composer.prototype.copyTexture = function( src, dst ) {
...
WAGNER.processShader = function( vertexShaderCode, fragmentShaderCode ) {
  ...
  var uniformType, uniformName, arraySize;
	...
	var vSPrefix = [ 'precision highp float;',
'precision highp int;',
'',
...
'attribute vec2 uv;' ];
	...
	var fsPrefix = [ 'precision highp float;', 'precision highp int;' ];

	WAGNER.log( 'Uniform reflection completed. Compiling...' );

	var shader = new THREE.RawShaderMaterial( {
		uniforms: uniforms,
		vertexShader: vSPrefix.join( '\r\n' ) + '\r\n\r\n' + vertexShaderCode,
		fragmentShader: fsPrefix.join( '\r\n' ) + '\r\n\r\n' + fragmentShaderCode,
...
WAGNER.Pass.prototype.loadShader = function( id, c ) {
	...
	WAGNER.loadShader( WAGNER.fragmentShadersPath + '/' + id, function( fs ) {
		self.shader = WAGNER.processShader( vs.join( '\r\n' ), fs );
...
WAGNER.Pass.prototype.isLoaded = function() {
	...
	if( this.loaded === null ) {
		if( this.shader instanceof THREE.ShaderMaterial || this.shader instanceof THREE.RawShaderMaterial ) {
			this.loaded = true;
		}
	} else {
		return this.loaded;
